<script lang="ts">
  ...
  import { ChevronRight, Info, PlayCircle } from '@lucide/svelte';
...
<div class="flex flex-col">
  ...
  <a class="mb-8 flex w-full cursor-pointer items-center justify-between rounded-xl bg-emerald-700 p-4 transition-colors hover:bg-emerald-500" href="/{selection?.id}">
  ...
  <div class="mb-8">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      {#each cards as card, i (i)}
        <ModuleCard onclick={() => (selection = card)} title={card.title} description={card.description} active={selection === card} />
      {/each}
    </div>
  </div>
</div>

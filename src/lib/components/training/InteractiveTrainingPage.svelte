<script lang="ts" generics="T extends TrainingItem">
  ...
  import InteractiveTrainingMain from './InteractiveTrainingMain.svelte';
  ...
  const modeConfig = {
    chords: { color: 'emerald', icon: Music, title: 'Interactive Chord Recognition' },
    intervals: { color: 'purple', icon: Target, title: 'Interactive Interval Training' },
    inversions: { color: 'blue', icon: RotateCcw, title: 'Interactive Chord Inversions' },
    progressions: { color: 'orange', icon: Volume2, title: 'Interactive Chord Progressions' }
  };
  ...
  <div class="max-w-7xl mx-auto">
      ...
      <header class="relative z-10 mb-8">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            ...
            <p class="text-gray-400">Click-based training with instant feedback</p>
          </div>
        </div>
      </header>
    </div>
    ...
    <div class="relative">
      {#key currentView}
        <div transition:slide={{ axis: 'y', duration: 300 }} class="relative z-10">
          {#if currentView === 'main'}
            <InteractiveTrainingMain {mode} {config} />
    ...
    <div class="mt-16 border-t border-gray-800 pt-6 text-center">
      <div class="flex items-center justify-center space-x-2 mb-2">
        ...
        <span class="text-sm font-medium text-gray-400">Interactive Ear Training</span>
      </div>
      <p class="text-xs text-gray-500">Click, learn, and improve with instant feedback</p>
    </div>
  </div>
</div>
